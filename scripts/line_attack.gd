...
func init(_position=position, _angle=theta, _timer=timer, _linger=linger, _type=type):
	position = _position
	theta = _angle
	timer = _timer
	linger = _linger
...
func spin():
	...
	tween.tween_callback(enable_collision_shape)
	tween.tween_property(self,"rotation", PI, linger).set_trans(Tween.TRANS_LINEAR)
